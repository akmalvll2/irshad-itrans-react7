...
import file from '../../../assets/template/job-template.xlsx'

import packageJson from '../../../../package.json'
const { config } = packageJson

const ImportJob = ({ visible, setVisible }) => {
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      for (var x = 0; x < data.length; x++) {
        const jobTitle = data[x].job_title
        const jobGrade = data[x].job_grade
        const jobDesc = data[x].job_scope
        await axios.post(`${config.REACT_APP_API_ENDPOINT}/addposition`, {
          jobTitle,
          jobGrade,
          jobDesc,
        })
  ...
  return (
    <>
      <CModal visible={visible} onClose={() => setVisible(false)} backdrop="static">
        <CModalHeader>IMPORT DATA (JOB PROFILE)</CModalHeader>
        <CModalBody>
        ...
        <CModalFooter>
          ...
          <CButton color="link">
            <a href={file} download={'Job Profile Template'}>
              Download Template
            </a>
          </CButton>
        </CModalFooter>
